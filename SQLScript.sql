...
CREATE TABLE PhieuCham (
    ...
    Loai NVARCHAR(10) NOT NULL, -- 'DETAI' | 'CHUYENDE'

    MaDT NVARCHAR(20) NULL,
    MaCD NVARCHAR(20) NULL,

    Vong INT NULL,              -- chỉ dùng cho CHUYÊN ĐỀ

    Diem DECIMAL(4,2) NOT NULL,

    CONSTRAINT FK_PC_HD FOREIGN KEY (MaHD) REFERENCES HoiDong(MaHD),
    CONSTRAINT FK_PC_CB FOREIGN KEY (MaCB) REFERENCES GiangVien(MaCB),
    CONSTRAINT FK_PC_DT FOREIGN KEY (MaDT) REFERENCES DeTai(MaDT),
    CONSTRAINT FK_PC_CD FOREIGN KEY (MaCD) REFERENCES ChuyenDe(MaCD),

    -- RÀNG BUỘC LOGIC
    CONSTRAINT CK_PC_Loai CHECK (
        (Loai = 'DETAI' AND MaDT IS NOT NULL AND MaCD IS NULL AND Vong IS NULL)
     OR (Loai = 'CHUYENDE' AND MaCD IS NOT NULL AND MaDT IS NULL AND Vong IS NOT NULL)
    ),

    -- UNIQUE cho ĐỀ TÀI
    CONSTRAINT UQ_PC_DT UNIQUE (MaHD, MaCB, MaDT),

    -- UNIQUE cho CHUYÊN ĐỀ
    CONSTRAINT UQ_PC_CD UNIQUE (MaHD, MaCB, MaCD, Vong)
);
